...
formula <- formula(paste0("discount_rate_1y~nihtbx_totalcomp_uncorrected_0y+cpeur2+eaeur1+iqeur2+", cov_list, 
                            '| rh_adi_perc1_0y | section8_0y+nihtbx_totalcomp_uncorrected_0y+cpeur2+eaeur1+iqeur2+', cov_list))
...
for (y in outcomes) {
  formula <- formula(paste0(y, "~nihtbx_totalcomp_uncorrected_0y+cpeur2+eaeur1+iqeur2+", cov_list, 
                            '| rh_adi_perc1_0y | section8_0y+nihtbx_totalcomp_uncorrected_0y+cpeur2+eaeur1+iqeur2+', cov_list))
  res<-ivreg(formula, data = data)
  beta<-as.matrix(round(summary(res)$coefficients[2,1],4))
  se<-as.matrix(round(summary(res)$coefficients[2,2],4))
...
openxlsx::write.xlsx(res_all, rowNames=TRUE, "~DD IV Regression_results.xlsx")
