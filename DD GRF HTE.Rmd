------
Model 1: Demographics+Delay Discounting
------
...
for (i in 1:100) {
  ...
  # tau.hat_i <- predict(forest_list[[i]])$predictions
  # aipw.scores_i<-get_scores(forest_list[[i]], num.trees.for.weights = 1000)
  # tau.hat.res<-cbind(tau.hat.res, data.frame(tau.hat_i))
  # aipw.scores.res<-cbind(aipw.scores.res, data.frame(aipw.scores_i))
...
ate<-average_treatment_effect(big_rf, num.trees.for.weights = 5000)
est <- ate['estimate']
se <- ate['std.err']
tstat <- est / se
...
ate.results <- as.data.frame(cbind(est, se, tstat, pvalue, lowci, highci))
colnames(ate.results) <- c("Estimate", "Std.err", "t value", "Pr(>t)", "95% LowCI", "95% High CI")
rownames(ate.results) <- "Average Treatment Effects"
ate.results


## Test Calibration
diff_forest<-transpose(data.frame(test_calibration_iv(big_rf)[2,1:4]))
lowci <- diff_forest[1] + -1 * qnorm(0.975) * diff_forest[2]
highci <- diff_forest[1] + 1 * qnorm(0.975) * diff_forest[2]
diff_forest <- cbind(diff_forest, lowci, highci)
colnames(diff_forest) <- c("Estimate", "Std.err", "t value", "Pr(>t)", "95% LowCI", "95% High CI")
rownames(diff_forest) <- "Differential Forest Prediction"
diff_forest
...
for (fold in seq(num.folds)) {
    tau.quantiles <- quantile(tau.hat[folds == fold], probs = seq(0, 1, by=1/num.rankings))
    ranking[folds == fold] <- cut(tau.hat[folds == fold], tau.quantiles, 
                                include.lowest=TRUE, labels=seq(num.rankings)) 
  }

ols <- lm(aipw.scores ~ 0 + factor(ranking))
forest.ate <- data.frame(paste0("Q", seq(num.rankings)), coeftest(ols, vcov=vcovHC(ols, "HC2"))[,])
colnames(forest.ate) <- c("Ranking", "Estimate", "Std.err", "t value", "Pr(>t)")
rownames(forest.ate) <- NULL # just for display

ate.lowci<- forest.ate$Estimate + -1 * qnorm(0.975) * forest.ate$Std.err 
ate.highci<- forest.ate$Estimate + 1 * qnorm(0.975) * forest.ate$Std.err
forest.ate<-cbind(forest.ate, ate.lowci, ate.highci)
colnames(forest.ate) <- c("Ranking", "Estimate", "Std.err", "t value", "Pr(>t)", "95% Lower CI", "95% Upper CI")
...
res<-summary(ols)$coefficients[2:10,]
rows <- paste("Rank", 2:num.rankings, "- Rank 1") # just for display
rownames(res) <- NULL
res <- data.frame(rows, res)
colnames(res) <- c("Ranking", "Estimate", "Std.err", "t value", "Pr(>t)")

gate.lowci<- res$Estimate + -1 * qnorm(0.975) * res$Std.err 
gate.highci<- res$Estimate + 1 * qnorm(0.975) * res$Std.err
res<-cbind(res, gate.lowci, gate.highci)
colnames(res) <- c("Ranking", "Estimate", "Std.err", "t value", "Pr(>t)", "95% Lower CI", "95% Upper CI")
res


## Subset the dataframe for "rank==1" and "rank==5"
resilient <- data[ranking == 1, ]
vulnerable <- data[ranking == 10, ]


## Associations between covariates & heterogeneity
covariates<-c('age_0y', 'bmi_0y', 'income_0y', 
          'high_educ_0y', 'sex_0y_F', 'vol',
          # 'married_0y_2', 
          'married_0y_3', 'married_0y_4', 'married_0y_5', 'married_0y_6',
          'parent_identity_0y_2','parent_identity_0y_3', 
          'parent_identity_0y_4', 'parent_identity_0y_5',
          'race_ethnicity_0y_2', 'race_ethnicity_0y_3', 
          # 'race_ethnicity_0y_4',
...
resilient$binary_group <- 0
vulnerable$binary_group <- 1
binary_df <- rbind(resilient, vulnerable)

model <- glm(formula=paste0("binary_group ~", paste0(covariates, collapse="+")), data = binary_df, family = binomial)
summary(model)
beta<-as.matrix(round(summary(model)$coefficients[2:22, 1],4))
se<-as.matrix(round(summary(model)$coefficients[2:22,2],4))
p<-as.matrix(round(summary(model)$coefficients[2:22,4],4))
p_fdr<-round((p.adjust(as.matrix(p), method = 'fdr')),4)
lowci<- beta + -1 * qnorm(0.975) * se 
highci<- beta + 1 * qnorm(0.975) * se
res_all<-cbind(beta, se, lowci, highci, p, p_fdr)
colnames(res_all)<-c("Estimates", "Std.Err", "95% Lower CI", "95% Upper CI", "P-value", "P-FDR")
rownames(res_all)<-covariates
res_all


model2 <- glm(formula=paste0("ranking ~", paste0(colnames(X), collapse="+")), data = data, family = gaussian)
summary(model2)
beta<-as.matrix(round(summary(model2)$coefficients[2:24, 1],4))
se<-as.matrix(round(summary(model2)$coefficients[2:24,2],4))
p<-as.matrix(round(summary(model2)$coefficients[2:24,4],4))
p_fdr<-round((p.adjust(as.matrix(p), method = 'fdr')),4)
lowci<- beta + -1 * qnorm(0.975) * se 
highci<- beta + 1 * qnorm(0.975) * se 

res_all2<-cbind(beta, se, lowci, highci, p, p_fdr)
colnames(res_all2)<-c("Estimates", "Std.Err", "95% Lower CI", "95% Upper CI", "P-value", "P-FDR")
rownames(res_all2)<-colnames(X)
res_all2


## Variable Importance
var_imp <- round(c(variable_importance(big_rf)), 10)
...
list_res<-list(VarImp=sorted_var_imp, Covariates=res_all2, 
               GATE=forest.ate, GATE_Diff=res, Covariates_logit=res_all, Monotonicity=monotone_test, 
               Alternative=alt_test, ANOVA=anova_res)
openxlsx::write.xlsx(list_res, rowNames=TRUE, paste0("C:\\Users\\socra\\Desktop\\Brain discounting\\GRF Results_Calibration\\", y, "_dd_3.xlsx"))
